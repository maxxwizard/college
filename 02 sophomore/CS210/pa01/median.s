...
# PRINT TEXT
#------------------
	la $a0, text0		# print "The array is:  "
	li $v0, 4
	syscall
#------------------
# PRINT CURRENT ARRAY
...
#------------------
	...
	la $t0, size		# $t0 = &size
	lw $a1, 0($t0)		# $a1 = size
	jal Median		# invoke Median method
	move $a0, $v0
	li $v0, 1
	syscall
#------------------
...
# PRINT TEXT
#------------------
	la $a0, text1		# print "The sorted array is:  "
	li $v0, 4
	syscall
#------------------
# PRINT CURRENT ARRAY
...
Median:
	addi $sp, $sp, -12	# malloc $sp for 3 words
	sw $s0, 8($sp)		# preserve $s0, $s1, $ra
	sw $s1, 4($sp)
	sw $ra, 0($sp)
	jal Sort		# call Sort method
	srl $t0, $a1, 1		# $s1 = Arr[n/2]
	sll $t9, $t0, 2
	add $t9, $a0, $t9
	lw $s1, 0($t9)
	addi $t9, $t9, -4	# $s0 = Arr[n/2-1]
	lw $s0, 0($t9)
	li $t0, 2
	div $a1, $t0		# $t1 = (n % 2)
	mfhi $t1
	beq $t1, $zero, even 	# if (n % 2) == 0, go to finish3
odd:	move $v0, $s1		# return B
	j finish3
even:	sub $t0, $s1, $s0	# return A + (B-A)/2
	li $t1, 2
	div $t0, $t1
	mflo $t2
	add $v0, $s0, $t2
	j finish3
finish3:lw $s0, 8($sp)		# restore $s0, $s1, $ra
	lw $s1, 4($sp)
	lw $ra, 0($sp)
	addi $sp, $sp, 12	# dealloc $sp for 3 words
...
##################################################
	...
	.data
Array:	.word 1, 2, 5, 7, 8, 10
size:	.word 6
CLRF:	.asciiz "\n"
space:	.asciiz " "
text0:	.asciiz "The array is:   "
text1:	.asciiz "The sorted array is:   "
